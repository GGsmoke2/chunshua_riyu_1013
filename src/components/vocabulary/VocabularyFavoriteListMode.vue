...
const playbackRate = ref<string>('1.0') // 播放速度，默认'1.0'x
...
// 监听单词数量变化，设置默认播放速度
watch(() => props.words.length, (newLength) => {
  // 根据单词数量设置默认播放速度
  if (newLength > 0) {
    // 默认保持1.0x，但可以根据单词数量调整
    // 这里保持默认1.0x，但您可以根据需要添加逻辑
